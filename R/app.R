          ...
          div(
            ...
            tabsetPanel(
              ...
              tabPanel(
                ...
                tags$p(
                  "By default, volcano plot module uses adjusted P-value. Untick if use P-value.",
              ...
              tabPanel(
                ...
                tags$p(
                  "NOTE: Remember to change manually cut-off values the in caption.",
                  style = "font-size: 16px; font-weight: bold;"
                ),
              ...
              tabPanel(
                ...
                colourInput("color_DownFDR_Volcano", "(Adjusted) P-value, Down-regulation Color:", value = "#BCF1B9"),
                colourInput("color_UpFDR_Volcano", "(Adjusted) P-value, Up-regulation:", value = "#FEB0B0"),
                colourInput("color_DownFDR_FC_Volcano", "(Adjusted) P-value & FC, Down-regulation):", value = "#5FDD59"),
                colourInput("color_UpFDR_FC_Volcano", "(Adjusted) P-value & FC, Up-regulation):", value = "#FE5E5E")
              ...
              tabPanel(
                ...
                tags$p(
                  "NOTE: If used, it needs to set both min and max values.",
                  style = "font-size: 16px; font-weight: bold;"
                ),
          ...
          div(
            ...
            tabsetPanel(
              ...
              tabPanel(
                ...
                tags$p(
                  "TIPS: If the ellipse were disrupted, set broader ranges.",
                  style = "font-size: 16px; font-weight: bold;"
                )
              ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$reload_tab_button_BoxPlot, {
    ...
    toastr_info(title = "NOTE", message = "Please re-upload your data files.", timeOut = 4500,
                position = "bottom-right", closeButton = TRUE)#, progressBar = TRUE)
  ...
  output$download_BubblePlot_ExampleORA_1 <- downloadHandler(
    filename = function() {
      "BubblePlot_DotPlot_ExampleORA_1.csv"  # Name of the file to be downloaded
    },
    content = function(file) {
      # Download the file from Google Drive and save it to 'file'
      download.file(
  ...
  output$download_BubblePlot_ExampleORA_2 <- downloadHandler(
    filename = function() {
      "BubblePlot_DotPlot_ExampleORA_2.csv"  # Name of the file to be downloaded
    },
    content = function(file) {
      # Download the file from Google Drive and save it to 'file'
      download.file(
  ...
  observeEvent(input$scoreFile_ScorePlot, {
    ...
    if (!is.null(values$metadata_ScorePlot)) {
      ...
      output$groupLevelSelector_ScorePlot <- renderUI({
        selectInput("groupLevels_selected_ScorePlot", "Select and Order Group Levels",
    ...
    toastr_info(title = "NOTE", message = "If you did not see the plot or saw any red ERROR message. Please load accurate input data.", timeOut = 12000,
                position = "bottom-right", closeButton = TRUE)#, progressBar = TRUE)

    # Check matched sample names or not
    if (!(identical(sort(values$score_ScorePlot$Sample), sort(values$metadata_ScorePlot$Sample)))) {
      toastr_error("Sample names did not match between 2 files.", position = "bottom-right", timeOut = 12000)
    }
  ...
  output$dynamicColorInputs_ScorePlot <- renderUI({
    ...
    color_inputs_ScorePlot <- lapply(seq_along(input$groupLevels_selected_ScorePlot), function(i) {
      ...
      colourInput(inputId = paste0("color_ScorePlot_", level), label = paste("Color for", level, "Group:"), value = default_color)
    })
    ...
    if (ncol(values$volcanoData) == 5) {
      ...
      if (input$checkbox_adjPvalue_Volcano) {
        ...
        values$volcanoData = values$volcanoData %>%
          setNames(c("Features", "FC", "log2FoldChange", "FDR", "minus_log10_FDR"))
      } else {
        # P-value
        values$volcanoData = values$volcanoData %>%
          setNames(c("Features", "FC", "log2FoldChange", "FDR", "minus_log10_Pvalue")) #pseudo FDR name to avoid to modify the below pre-coded variable name
      }
    }
  ...
  observeEvent(input$expressionFile_BoxPlot, {
    ...
    toastr_info(message = "Ensure: (1) 2 columns in metadata; (2) identical sample names between files; and (3) not duplicated features.", timeOut = 15000,
                position = "bottom-right", closeButton = TRUE)
    toastr_info(title = "NOTE", message = "If you did not see the plot or saw any red ERROR message. Please load accurate input data.", timeOut = 15000,
                position = "bottom-right", closeButton = TRUE)#, progressBar = TRUE)
    ...
    tryCatch({
      ...
      if (!(identical(sort(names(values$expression_BoxPlot)), sort(values$metadata_BoxPlot$Sample)))) {
        toastr_error("Sample names did not match between 2 files.", position = "bottom-right", timeOut = 15000)
      }

    }, error = function(e) {
      # If an error occurs, show a failure message
      toastr_error(title = "ERROR", "Data containe duplicated features.", position = "bottom-right", timeOut = 15000)
  ...
  output$dynamicColorInputs_BoxPlot <- renderUI({
    ...
    color_inputs_BoxPlot <- lapply(seq_along(input$groupLevels_selected_BoxPlot), function(i) {
      ...
      colourInput(inputId = paste0("color_BoxPlot_", level), label = paste("Color for", level, "Group:"), value = default_color)
    })
  ...
  observeEvent(input$NormDataFile_HeatmapSimple, {
    ...
    tryCatch({
      ...
      # Check matched sample names or not
      if (!(identical(sort(names(values$NormData_HeatmapSimple)), sort(values$metadata_HeatmapSimple$Sample)))) {
        toastr_error("Sample names did not match between 2 files.", position = "bottom-right", timeOut = 15000)
      }
  ...
  output$dynamicColorInputs_HeatmapSimple <- renderUI({
    ...
    color_inputs_HeatmapSimple <- lapply(seq_along(input$groupLevels_selected_HeatmapSimple), function(i) {
      ...
      colourInput(inputId = paste0("color_HeatmapSimple_", level), label = paste("Color for", level, "Group:"), value = default_color)
    })
